...
\begin{flalign}
        \phi(z) &= \frac{1}{2}[ \tanh(\gamma(x + 0.5L_z)) -\tanh(\gamma(x - 0.5L_z)) ] \\
        \phi(z) &= \frac{2}{\pi}\left[ \arctan \left( e^{\gamma (-x + 0.5L_z)} \right) + \arctan \left( e^{\gamma (x + 0.5L_z)} \right) \right]- 1 
\end{flalign}
\begin{figure}[htbp]
    \centering
...
\begin{flalign}
    ...
    \\&=\mathcal{F}_{3D}(E_{New-2D}.B_{3D})(p_x,p_y,p_z)
\end{flalign}
where the new screening factor array $E_{New-2D}$ is defined as
\begin{flalign}
    E_{New-2D}(k_x,k_y,k_z) &= \frac{\sqrt{\pi}}{L_xL_y}\left[ \int_{0}^{\alpha}\frac{dt}{t^2}C_{k_z}(t){exp}\left(\frac{-1}{4t^2}|\vec G|_{xy}^2\right)\right] \label{eq:N2D-ScreenFunction}
\end{flalign}
